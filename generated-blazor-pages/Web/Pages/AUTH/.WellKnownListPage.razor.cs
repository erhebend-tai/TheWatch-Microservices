...
public partial class .WellKnownListPage
{
    ...
    private async Task LoadDataAsync()
    {
        ...
        try
        {
            ...
            // items = await getJwksService.GetAllAsync();
    ...
    private void OnView(object item)
    {
        ...
        Navigation.NavigateTo($"//.well-known/jwks.json/{item.GetType().GetProperty("Id")?.GetValue(item)}");
    }

    private void OnEdit(object item)
    {
        // TODO: Navigate to edit page
        Navigation.NavigateTo($"//.well-known/jwks.json/{item.GetType().GetProperty("Id")?.GetValue(item)}/edit");
    }
